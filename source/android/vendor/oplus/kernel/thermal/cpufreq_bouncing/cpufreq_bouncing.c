...
struct cpufreq_bouncing {
	...
	int freq_sorting;
	int freq_levels;
	unsigned int freqs[NR_FREQ];

	/* trace info */
	long long freqs_resident[NR_FREQ];
...
static int update_delay = 8 * NSEC_PER_MSEC;
...
static int cb_main(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if (self_activate) {
			usleep_range(MSEC_TO_USEC(sleep_range_ms[0]), MSEC_TO_USEC(sleep_range_ms[1]));
		}
	}
