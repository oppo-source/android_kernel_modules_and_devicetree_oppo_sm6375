...
static const struct snd_kcontrol_new audio_extend_common_controls[] = {
#ifdef OPLUS_FEATURE_EAR_PROTECTION
...
static int extend_codec_prop_parse(struct device *dev, const char *codec_prop[], struct codec_prop_info *codec_info)
{
	...
	if (ret) {
		...
		if (tdm_port_id_flag) {
			set_smartpa_info(tdm_port_id_flag, g_extend_pdata->spk_pa_info->i2s_id);
		}
	}
	#endif /*OPLUS_ARCH_EXTENDS*/
...
int audio_extend_add_common_contorls(struct snd_soc_component *component)
{
	int ret = 0;

	ret = snd_soc_add_component_controls(component, audio_extend_common_controls,
									ARRAY_SIZE(audio_extend_common_controls));
	if (ret < 0) {
...
EXPORT_SYMBOL(audio_extend_add_common_contorls);
