...
static int tfa98xx_send_volume(uint8_t channel, uint8_t volume)
{
	...
	#ifdef OPLUS_ARCH_EXTENDS
	ret = send_tfa_cal_in_band(&cmd[0], sizeof(cmd));
	if (ret) {
		pr_err("tfa98xx_send_volume fail , returning\n");
		return ret;
	}
...
#define STATUS_CHECK_MASK                  0xDE //mask bit2~4, bit6~7 (bit1 reserve 1)
...
static int tfa98xx_check_status_reg(void )
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		if ((Tfa98xx_Error_Ok == err) && (STATUS_NORMAL_VALUE != (reg_val&STATUS_CHECK_MASK))) {
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	#ifdef OPLUS_FEATURE_MM_FEEDBACK
	nr_controls += 1; /*TFA_CHECK_FEEDBACK*/
	#endif
	...
	#ifdef OPLUS_FEATURE_MM_FEEDBACK
	tfa98xx_controls[mix_index].name = "TFA_CHECK_FEEDBACK";
	tfa98xx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	tfa98xx_controls[mix_index].info = snd_soc_info_enum_double;
	tfa98xx_controls[mix_index].get = tfa98xx_get_check_feedback;
	tfa98xx_controls[mix_index].put = tfa98xx_set_check_feedback;
	tfa98xx_controls[mix_index].private_value = (unsigned long)&tfa98xx_check_feedback_enum;
	mix_index++;
	#endif /*OPLUS_FEATURE_MM_FEEDBACK*/
...
static int tfa98xx_send_mute_cmd(void)
{
	...
	ret = send_tfa_cal_in_band(&cmd[0], sizeof(cmd));
	if (ret) {
		pr_err("tfa98xx_send_mute_cmd fail, returning\n");
		return ret;
	}
