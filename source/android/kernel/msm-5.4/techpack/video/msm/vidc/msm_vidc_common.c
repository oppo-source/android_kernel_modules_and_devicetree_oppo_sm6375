...
//sunqinhua@MULTIMEDIA.MEDIASERVER.PLAYER.670039, 2020/12/15, add for video load
...
static int msm_vidc_check_mbps_supported(struct msm_vidc_inst *inst)
{
	...
	if (inst->state == MSM_VIDC_OPEN_DONE) {
		...
		max_image_load = inst->core->resources.max_image_load;
#ifdef OPLUS_ARCH_EXTENDS
//sunqinhua@MULTIMEDIA.MEDIASERVER.PLAYER.670039, 2020/12/15, add for video load issue
		max_video_load += NUM_MBS_PER_SEC(1920, 1088,40);
#endif
		if (video_load > max_video_load) {
			s_vpr_e(inst->sid,
