...
int oplus_panel_change_backlight_cure(struct dsi_panel *panel, u32 *bl_lvl)
{
	int rc = 0;
	u32 bl_lvls = *bl_lvl;

	if (NULL == panel) {
		pr_err("panel point is null\n");
		return rc;
	}
	if (!strcmp(panel->oplus_priv.vendor_name, "NT37800")) {
		if (bl_lvls < 400 && bl_lvls != 0) {
			bl_lvls = 400;
		}
	}
	if (!strcmp(panel->oplus_priv.vendor_name, "AMS643AG02")) {
		if (bl_lvls > panel->bl_config.bl_normal_max_level) {
			bl_lvls = bl_lvls + panel->bl_config.bl_hbm_min_level - panel->bl_config.bl_normal_max_level - 1;
			bl_lvls = bl_lvls < panel->bl_config.bl_max_level ? bl_lvls : panel->bl_config.bl_max_level;
		}
	}
	*bl_lvl = bl_lvls;
	return rc;
