	...
	u32 bl_lvl)
{
	...
	if (OPLUS_DISPLAY_AOD_SCENE != get_oplus_display_scene()) {
		...
		if(!strcmp(panel->oplus_priv.vendor_name, "AMS644VA04") || !strcmp(panel->oplus_priv.vendor_name, "AMS643AG02")) {
			if (bl_lvl > panel->bl_config.bl_normal_max_level) {

				rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_HBM_ENTER_SWITCH);
				DSI_INFO("Enter original HBM and bl_normal_max_level = %d\n", panel->bl_config.bl_normal_max_level);
				if (rc < 0)
	...
	rc = oplus_panel_change_backlight_cure(panel, &bl_lvl);
	if (rc < 0)
		DSI_ERR("failed to change dcs backlight:%d\n", bl_lvl);
