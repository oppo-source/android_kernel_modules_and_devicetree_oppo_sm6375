...
/* 0xCABB = 1100101010111011b, mask bit0/1/3/4/5/7/9/11/14/15 */
...
static const struct snd_kcontrol_new aw882xx_check_feedback[] = {
	...
	SOC_SINGLE_EXT("SpkrRight PA_ERR_CHECK_FEEDBACK", SND_SOC_NOPM, 0, 0xff, 0,
			aw882xx_get_check_feedback, aw882xx_set_check_feedback),
};
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = aw882xx->iv_output;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0])
		aw882xx->iv_output  = false;
	else
		aw882xx->iv_output  = true;

	if (aw882xx->aw_pa->chip_id == PID_2308_ID) {
		aw882xx_dev_iv_forbidden_output(aw882xx->aw_pa, aw882xx->iv_output);
	}
...
static int aw882xx_get_f0_en(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int ret;
	struct aw_device *local_dev = NULL;
	...
	ret = aw882xx_dev_get_list_head(&dev_list);
	if (ret) {
		aw_dev_err(aw882xx->dev, "get dev list failed");
		return ret;
	}

	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		cali_f0_en = cali_f0_en || local_dev->cali_desc.cali_f0_en;
	}

	ucontrol->value.integer.value[0] = cali_f0_en;

	aw_dev_dbg(aw882xx->dev, "cali_f0_en is %d", cali_f0_en);
	return 0;
}
...
static struct snd_kcontrol_new aw882xx_controls[] = {
	...
	SOC_ENUM_EXT("aw882xx_f0_switch", aw882xx_snd_enum[0],
		aw882xx_get_f0_en, NULL),
};
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
/*Add for smartpa err feedback.*/
	if (aw882xx->aw_pa->channel < (sizeof(aw882xx_check_feedback) / sizeof(aw882xx_check_feedback[0]))) {
		aw_componet_codec_ops.add_codec_controls(aw882xx->codec,
				&aw882xx_check_feedback[aw882xx->aw_pa->channel], 1);
	}
/* 2024/07/08, Add for smartpa vbatlow err check. */
	aw882xx->vbatlow_cnt = 0;
...
static DEVICE_ATTR_WO(voltage_offset);
...
static ssize_t aw882xx_reg_read_test(struct file *file,
				...
				loff_t *ppos)
{
	...
	int len = 0;
	char *str = NULL;
	unsigned char rw_reg_addr = 0x00; /*reg addr of chip ID*/
	...
	if (!i2c) {
		aw_pr_err("%s:i2c is error", __func__);
		return ret;
	}

	aw882xx = i2c_get_clientdata(i2c);
	if (!aw882xx) {
		aw_pr_err("%s:aw882xx is error", __func__);
		return ret;
	}
	aw_pr_info("enter:%s reg addr=0x%x", __func__, i2c->addr);

	str = kzalloc(REG_READ_LEN, GFP_KERNEL);
	if (!str) {
		aw_pr_err("%s:memory allocation failed\n", __func__);
		return ret;
	}

	if (aw882xx->aw_pa->ops.aw_check_rd_access(rw_reg_addr)) {
		ret = aw882xx_i2c_read(aw882xx, rw_reg_addr, &reg_val);
		if (!ret && reg_val == aw882xx->aw_pa->chip_id) {
			len = snprintf(str, REG_READ_LEN, "0x%x\n", reg_val);
		} else {
			len = snprintf(str, REG_READ_LEN, "error\n");
		}
		aw_pr_info("%s: ret=%d, len=%d, str=%s", __func__, ret, len, str);
	}

	if (len == 0) {
		aw_pr_err("%s: str copy err\n", __func__);
		kfree(str);
		return ret;
	}

	ret = simple_read_from_buffer(user_buf, count, ppos, str, strlen(str));
	aw_pr_info("exit %s: ret=%d", __func__, ret);
	kfree(str);
	return ret;
}

static const struct proc_ops aw882xx_i2c_test_fops = {
	.proc_open = simple_open,
	.proc_read = aw882xx_reg_read_test,
