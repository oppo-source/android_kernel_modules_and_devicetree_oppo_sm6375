...
void cgroup_set_sched_assist_boost_task(struct task_struct *p, char *comm)
{
	...
	if (unlikely(cgroup_check_set_sched_assist_boost(comm, p))) {
		...
		if (is_top(p) || test_task_ux(p->group_leader)) {
			if (oplus_get_im_flag(p->group_leader) == IM_FLAG_LAUNCHER)
				oplus_set_ux_state_lock(p, (ux_state | SA_TYPE_LIGHT), -1, true);
			else
				oplus_set_ux_state_lock(p, (ux_state | SA_TYPE_HEAVY), -1, true);
		}
		else
			oplus_set_ux_state_lock(p, (ux_state & ~SA_TYPE_HEAVY), -1, true);
	}
...
void sched_assist_target_comm(struct task_struct *task, const char *buf)
{
	...
	if (!strncmp(comm, "C2OMXNode", 9) || !strncmp(comm, "MP4WtrAudTrkThr", 15) || !strncmp(comm, "MP4WtrVidTrkThr", 15)) {
		ux_state = oplus_get_ux_state(task);
		oplus_set_ux_state_lock(task, (ux_state | SA_TYPE_ANIMATOR), -1, true);
	}
#endif
}
