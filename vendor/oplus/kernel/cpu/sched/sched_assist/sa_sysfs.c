...
/* if this task set as ux from userspace, we promise it's low-latency in hmbird-sched */
static void write_ux_task_sched_prop(struct task_struct *task)
{
	int ux_state = 0;
	unsigned long old_sp, old_deadline_sp, old_clr_deadline_sp;
	unsigned long new_deadline_sp = SCHED_PROP_DEADLINE_LEVEL4;
	...
	old_sp = sched_get_sched_prop(task);
	old_deadline_sp = (old_sp & SCHED_PROP_DEADLINE_MASK);
	old_clr_deadline_sp = (old_sp & ~SCHED_PROP_DEADLINE_MASK);

	ux_state = oplus_get_ux_state(task);
	if (ux_state & SCHED_ASSIST_UX_MASK) {	/* static ux */
		if (old_deadline_sp == 0) {
			sched_set_sched_prop(task, old_clr_deadline_sp | new_deadline_sp);
			sched_set_sp_sync_ux(task, true);
		}
	} else {
		if (sched_get_sp_sync_ux(task) && (old_deadline_sp == new_deadline_sp)) {
			sched_set_sp_sync_ux(task, false);
			sched_set_sched_prop(task, old_clr_deadline_sp);
		} else if (old_deadline_sp == 0) {
			sched_set_sp_sync_ux(task, false);
		}
	}

end:
	if (unlikely(global_debug_enabled & DEBUG_SET_SCHEDPROP)) {
		pr_info("hmbird_set: ux_state = 0x%x prio = %d sp = %lu current(pid = %d comm = %s) set(pid = %d tgid = %d comm = %s) \
			old_clr_deadline_sp = %lu old_sp = %lu new_sp = %lu sp_sync_ux = %d\n", ux_state, task->prio, new_deadline_sp,
			current->pid, current->comm, task->pid, task->tgid, task->comm, old_clr_deadline_sp,
			old_sp, sched_get_sched_prop(task), sched_get_sp_sync_ux(task));
	}
}
...
static long write_task_ux(pid_t pid, pid_t tid, int ux_value, bool fromSysOrApp)
{
	...
	if (ux_task) {
		...
		if (need_update) {
			...
			write_ux_task_sched_prop(ux_task);
		}
