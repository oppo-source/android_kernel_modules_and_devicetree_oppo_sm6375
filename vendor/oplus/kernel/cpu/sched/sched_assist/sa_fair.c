...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FRAME_BOOST)
unsigned int sysctl_slide_boost_enabled_ux = 0;
EXPORT_SYMBOL_GPL(sysctl_slide_boost_enabled_ux);
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FRAME_BOOST)
static inline bool slide_cfs_boost(struct task_struct *task)
{
	return sysctl_slide_boost_enabled_ux && test_sched_assist_ux_type(task, SA_TYPE_HEAVY);
}
#endif
...
static inline int get_task_cls_for_scene(struct task_struct *task)
{
	...
	if (slide_cfs_boost(task))
		return !task_fits_capacity(task, ux_cputopo.sched_cls[0].capacity) ? cls_mid : 0;
...
inline void oplus_check_preempt_wakeup(struct rq *rq, struct task_struct *p, bool *preempt, bool *nopreempt)
{
	...
	if (is_audio_scene()) {
		...
		curr_ux = (oplus_get_im_flag(curr) == IM_FLAG_AUDIO_CAMERA_HAL) ? false : test_task_ux(curr);
	}
	else {
