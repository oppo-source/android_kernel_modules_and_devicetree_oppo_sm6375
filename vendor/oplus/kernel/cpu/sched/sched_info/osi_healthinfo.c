...
static void init_atomic(void)
{
	...
	for (i = 0; i < task_struct_max; ++i) {
		arch_atomic_long_set(&maj_flt_stat[i], 0);
		arch_atomic_long_set(&min_flt_stat[i], 0);
	}
}
...
void android_vh_exit_check_pagefault_handler(void *unused, struct task_struct *p)
{
	...
	if (p->tgid == p->pid) {
		arch_atomic_long_add((long)p->maj_flt, &maj_flt_stat[0]);
		arch_atomic_long_add((long)p->min_flt, &min_flt_stat[0]);
		/*match task_struct->comm*/
		i = comm_match(p->comm);
		if (i > 0 && i < task_struct_max) {
			arch_atomic_long_add((long)p->maj_flt, &maj_flt_stat[i]);
			arch_atomic_long_add((long)p->min_flt, &min_flt_stat[i]);
		}
	}
	return;
}
