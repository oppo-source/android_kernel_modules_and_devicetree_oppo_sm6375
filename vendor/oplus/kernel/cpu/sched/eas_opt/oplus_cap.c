...
EXPORT_SYMBOL_GPL(eas_opt_enable);

int eas_opt_debug_enable = 0;
EXPORT_SYMBOL_GPL(eas_opt_debug_enable);

int sa_adjust_group_enable = 0;
EXPORT_SYMBOL_GPL(sa_adjust_group_enable);

int force_apply_ocap_enable = 0;
EXPORT_SYMBOL_GPL(force_apply_ocap_enable);
...
EXPORT_SYMBOL_GPL(real_cpu_cap);
EXPORT_SYMBOL_GPL(oplus_cap_multiple);
EXPORT_SYMBOL_GPL(nr_oplus_cap_multiple);
EXPORT_SYMBOL_GPL(back_cap);
...
bool adjust_group_task(struct task_struct *p, int cpu)
{
	...
	if ((group_adjust.task_compensate + util_p > real_cpu_cap[cpu]) || (margin + (int)util_p + (int)cpu_util_no_p > (int)real_cpu_cap[cpu])) {
		if (unlikely(eas_opt_debug_enable))
			trace_printk("[eas_opt]:id:%d, comm:%s, util_p:%lu, margin:%d, cpu_util_no_p:%lu, origin_cpu:%d, origin_cpu_cap:%lu\n",
...
void oplus_cap_systrace_c(unsigned int cpu, unsigned long cap_orig, unsigned long cap)
{
	...
	if (eas_opt_debug_enable && eas_opt_enable) {
		snprintf(buf_orig, sizeof(buf_orig), "C|10001|Cpu%d_oplus_cap_orig|%lu\n", cpu, cap_orig);
		fake_tracing_mark_write(buf_orig);
		snprintf(buf, sizeof(buf), "C|10001|Cpu%d_real_cap_orig|%lu\n", cpu, cap);
		fake_tracing_mark_write(buf);
	}
}
EXPORT_SYMBOL_GPL(oplus_cap_systrace_c);
		...
		size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < OPLUS_CPUS; i++) {
		len = snprintf(buffer, sizeof(buffer), "%scpu%d:%lu\n",
		...
		size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < AD_MAX_GROUP; i++) {
		len = snprintf(buffer, sizeof(buffer), "%sgroup_adjust_%s:%lu %d\n",
	...
	len = snprintf(buffer, sizeof(buffer), "%sadjust_std_vtime_slice:%llu\n", buffer, group_adjust.adjust_std_vtime_slice);
