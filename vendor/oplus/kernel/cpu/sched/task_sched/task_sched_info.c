...
void cpufreq_limit_notifier_init(struct cpufreq_policy *policy);
...
void cpufreq_limit_notifier_init(struct cpufreq_policy *policy)
{
	...
	if (!policy) {
		sched_err("cpufreq_limit_notifier_init:get cpu policy null.\n");
		return;
	}
...
static int task_sched_policy_notifier_callback(struct notifier_block *nb,
						unsigned long val, void *data)
{
	...
	if (val != CPUFREQ_CREATE_POLICY)
		return NOTIFY_DONE;
	cpufreq_limit_notifier_init(policy);
	return NOTIFY_OK;
}


static struct notifier_block task_sched_cpufreq_policy_notifier = {
	.notifier_call = task_sched_policy_notifier_callback,
};
...
int sched_info_init(void)
{
	...
	ret = cpufreq_register_notifier(&task_sched_cpufreq_policy_notifier,
					CPUFREQ_POLICY_NOTIFIER);
	if (ret)
		sched_err("register cpufreq nb fail\n");
