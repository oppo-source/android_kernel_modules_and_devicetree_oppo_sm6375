...
static bool is_game_scene = false;
...
static bool uag_gov_should_update_freq(struct uag_gov_policy *sg_policy, u64 time)
{
	...
	if ((sg_policy->flags & SCHED_CPUFREQ_DEF_FRAMEBOOST) ||
		(sg_policy->flags & SCHED_CPUFREQ_IMS_FRAMEBOOST) ||
								...
								unsigned int next_freq)
{
	...
	if (sg_policy->game_scene_limits_changed) {
		sg_policy->game_scene_limits_changed = false;
		return false;
	}
...
void set_soft_limit_freq(struct cpufreq_policy *policy, unsigned int soft_freq)
{
	...
	cpu_val_systrace_c(soft_freq, first_cpu, "frameboost_limit", frameboost_limit);
				...
				unsigned int flags)
{
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE))
		val_systrace_c(util, cpumask_first(policy->cpus), "raw_util", raw_util);
#ifdef CONFIG_OPLUS_MULTI_LV_TL
	...
	if (sg_policy->tunables->multi_tl_enable) {
		...
		if (unlikely(sysctl_frame_boost_debug) & DEBUG_SYSTRACE)
			cpu_val_systrace_c((sg_policy->multi_util[UA_UTIL_FBG] > util) ? 1 : 0, cpumask_first(policy->cpus), "fbg_active", fbg_active);
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FRAME_BOOST)
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE))
		cpu_val_systrace_c(util, cpumask_first(policy->cpus), "raw_util", raw_util);
#ifdef CONFIG_OPLUS_MULTI_LV_TL
	sg_policy->multi_util[UA_UTIL_SYS] = util;
	if (sg_policy->tunables->multi_tl_enable) {
...
static void uag_gov_limits(struct cpufreq_policy *policy)
{
	...
	if (is_game_scene)
		sg_policy->game_scene_limits_changed = true;
...
static ssize_t is_game_scene_proc_write(struct file *file,
	const char __user *buf, size_t count, loff_t *ppos)
{
	...
	is_game_scene = value == 1;

	return count;
}

static ssize_t is_game_scene_proc_read(struct file *file,
...
static const struct proc_ops is_game_scene_proc_ops = {
	.proc_write 	= is_game_scene_proc_write,
	.proc_read		= is_game_scene_proc_read,
	.proc_lseek 	= default_llseek,
};

static void create_is_game_scene_proc_file(struct proc_dir_entry *dir)
{
	proc_create("is_game_scene", 0664, dir, &is_game_scene_proc_ops);
}
