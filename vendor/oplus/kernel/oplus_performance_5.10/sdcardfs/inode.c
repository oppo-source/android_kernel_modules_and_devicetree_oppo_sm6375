...
static int sdcardfs_create(struct mnt_idmap *idmap, struct inode *dir, struct dentry *dentry,
			 umode_t mode, bool want_excl)
{
	...
	err = vfs_create(&nop_mnt_idmap,d_inode(lower_parent_dentry), lower_dentry, mode, want_excl);
	if (err)
		goto out;
...
static int touch(struct dentry *dir_dentry, char *abs_path, mode_t mode)
{
	...
	inode_lock(dir);
	err = vfs_create(&nop_mnt_idmap, dir, target_dentry, mode, true);
	inode_unlock(dir);
out_unlock:
	dput(target_dentry);
out:
...
static int sdcardfs_mkdir(struct mnt_idmap *idmap, struct inode *dir, struct dentry *dentry, umode_t mode)
{
	int err;
 ...
 */
static int sdcardfs_rename(struct mnt_idmap *idmap, struct inode *old_dir, struct dentry *old_dentry,
...
static int sdcardfs_permission(struct mnt_idmap *idmap, struct inode *inode, int mask)
{
	int err;
	...
	err = generic_permission(&nop_mnt_idmap,&tmp, mask);
	return err;
}

static int sdcardfs_setattr(struct mnt_idmap *idmap, struct dentry *dentry, struct iattr *ia)
{
	int err;
	struct dentry *lower_dentry;
	struct inode *inode;
	...
	err = setattr_prepare(&nop_mnt_idmap,&tmp_d, ia);
...
static int sdcardfs_getattr(struct mnt_idmap *idmap, const struct path *path, struct kstat *stat,
				u32 request_mask, unsigned int flags)
{
	...
	*/
	err = vfs_getattr(&lower_path, &lower_stat, request_mask, flags & ~AT_GETATTR_NOSEC);
	if (err)
		goto out;
