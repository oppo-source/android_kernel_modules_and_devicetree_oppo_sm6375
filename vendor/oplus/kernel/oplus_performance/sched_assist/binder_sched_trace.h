...
TRACE_EVENT(binder_ux_task,
	TP_PROTO(int sync, int pending_async, int set_ux, struct task_struct *ux_task, int ux_enable, struct binder_transaction *t,
		char *info),
	TP_ARGS(sync, pending_async, set_ux, ux_task, ux_enable, t, info),
	...
	TP_printk("sync=%d pending_async=%d set_ux=%d pid=%d tgid=%d comm=%s ux_enable=%d t=0x%llx info=%s",
		__entry->sync, __entry->pending_async, __entry->set_ux, __entry->pid, __entry->tgid, __entry->comm,
		__entry->ux_enable, (unsigned long long)__entry->t, __entry->info)
);

TRACE_EVENT(binder_ux_enable,
	TP_PROTO(struct task_struct *task, int enable, struct binder_transaction *t, char *info),
	TP_ARGS(task, enable, t, info),
	...
	TP_printk("task(pid=%d tgid=%d comm=%s) ux_enable=%d t=0x%llx info=%s",
