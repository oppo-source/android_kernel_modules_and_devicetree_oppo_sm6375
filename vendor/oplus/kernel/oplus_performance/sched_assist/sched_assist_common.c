...
bool test_task_identify_ux(struct task_struct *task, int id_type_ux)
{
	...
	if (id_type_ux == SA_TYPE_ID_CAMERA_PROVIDER) {
		...
		if(grp_leader) {
			get_task_struct(grp_leader);
			is_provider = grp_leader->ux_state & SA_TYPE_ID_CAMERA_PROVIDER;
			put_task_struct(grp_leader);
		}
		if (task && (task->ux_state & SA_TYPE_LISTPICK) && is_provider)
			return true;
