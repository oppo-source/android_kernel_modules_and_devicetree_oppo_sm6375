...
#if IS_ENABLED(CONFIG_OPLUS_SEPARATE_USBC_SWITCH)
/* 2022/11/02, Add for separate audio switch */
	if ((mbhc->mbhc_cfg->separate_aud_sw_support && mbhc->need_cross_conn)||
			(!mbhc->mbhc_cfg->separate_aud_sw_support && mbhc->need_cross_conn &&
				mbhc->mbhc_cfg->swap_gnd_mic)) {
#else
...
/* 2022/11/02, Add for separate audio switch */
			if (mbhc->mbhc_cfg->separate_aud_sw_support) {
				usbc_switch_hs_event(HS_MIC_GND_SWAP);
			} else {
...
#if IS_ENABLED(CONFIG_OPLUS_SEPARATE_USBC_SWITCH)
/* 2022/11/02, Add for separate audio switch */
	if (mbhc->mbhc_cfg->separate_aud_sw_support && !wcd_swch_level_remove(mbhc)) {
		usbc_switch_hs_event(HS_MIC_GND_SWAP);
		msleep(10);
		output_mv_second = wcd_measure_adc_continuous(mbhc);
		plug_type_second = wcd_mbhc_get_plug_from_adc(mbhc, output_mv_second);
		pr_info("%s: second check output_mv = %d, plug_type_second = %d\n",
			__func__, output_mv_second, plug_type_second);

		pr_info("%s: second check output_mv = %d, plug_type = %d\n",
			__func__, output_mv, plug_type);

		/* if first and second measure all headset, choose the bigger output mv;
			if one is headset, another not headset, choose the headset one;
			in other case, choose any one */
		if (((plug_type == MBHC_PLUG_TYPE_HEADSET) && \
				(plug_type_second == MBHC_PLUG_TYPE_HEADSET) && \
				(output_mv > output_mv_second)) || \
			((plug_type == MBHC_PLUG_TYPE_HEADSET) && \
				(plug_type_second != MBHC_PLUG_TYPE_HEADSET))) {
			/* swap gnd and mic back to first connection state*/
			usbc_switch_hs_event(HS_MIC_GND_SWAP);
			msleep(5);
		} else {
			/* choose second connection state */
			output_mv = output_mv_second;
