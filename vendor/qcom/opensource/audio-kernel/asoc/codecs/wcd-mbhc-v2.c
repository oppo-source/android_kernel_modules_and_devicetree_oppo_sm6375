...
/* 2022/11/02, Add for separate audio switch */
...
#if IS_ENABLED(CONFIG_OPLUS_SEPARATE_USBC_SWITCH)
/* 2022/11/02, Add for separate audio switch */
	if (mbhc->mbhc_cfg->separate_aud_sw_support) {
		pr_info("%s: status=%d, mask=%d\n", __func__, status, mask);

		if (WCD_MBHC_JACK_MASK == mask) {
			if (mask & status) {
				usbc_switch_hs_event(HS_LR_CONNECT);
			} else {
				usbc_switch_hs_event(HS_PLUG_OUT);
			}
		}
	}
#endif /* CONFIG_OPLUS_SEPARATE_USBC_SWITCH */
...
bool wcd_swch_level_remove(struct wcd_mbhc *mbhc)
{
	...
	bool ret;
	pr_debug("%s: enter, usbc_analog_status=%d\n",
		__func__, mbhc->usbc_analog_status);

	if (mbhc->mbhc_cfg->usbc_hs_g_det_support) {
		ret = (mbhc->usbc_analog_status) ? false : true;
	} else {
		WCD_MBHC_REG_READ(WCD_MBHC_SWCH_LEVEL_REMOVE, result2);
		ret = (result2) ? true : false;
	}
	pr_debug("%s: ret =%d\n", __func__, ret);
	return ret;
#else
				   ...
				   enum wcd_mbhc_plug_type plug_type)
{
	...
	if (mbhc->mbhc_cfg->usbc_hs_g_det_support &&
		...
		pr_info("%s: usbc_analog_status %d, cable already removed, exit\n",
			__func__, mbhc->usbc_analog_status);
		goto exit;
	}
...
#define MIN_IRQ_INTERNAL_MS        1500
#define CHECK_COUNT_DELAY_MS       3000
static int g_set_gpio_cnt = 0;
static int g_get_audio_cnt = 0;
static ktime_t g_last_irq_tm = 0;
static void hs_g_check_count_fn(struct work_struct *work)
{
	char buf[MM_KEVENT_MAX_PAYLOAD_SIZE] = {0};
	int usbc_cnt = get_usbc_event_cnt();
	int usbc_audio_cnt = get_usbc_audio_event_cnt();

	pr_info("%s: g_set_gpio_cnt=%d, usbc_cnt=%d, g_get_audio_cnt=%d, usbc_audio_cnt=%d\n",
		__func__, g_set_gpio_cnt, usbc_cnt, g_get_audio_cnt, usbc_audio_cnt);

	if (g_set_gpio_cnt > usbc_cnt) {
		pr_info("%s: g_set_gpio_cnt=%d > usbc_cnt=%d, not equal\n", __func__, g_set_gpio_cnt, usbc_cnt);
		scnprintf(buf, sizeof(buf) - 1, "func@@%s$$payload@@wcd set gpio count=%d,usbc_switch get event count=%d,not equal", \
				__func__, g_set_gpio_cnt, usbc_cnt);
		upload_mm_fb_kevent_to_atlas_limit(OPLUS_AUDIO_EVENTID_HEADSET_DET, buf, MM_FB_KEY_RATELIMIT_1MIN);
		g_set_gpio_cnt = 0;
		clear_usbc_event_cnt();
	} else if (g_set_gpio_cnt < usbc_cnt) {
		pr_info("%s: g_set_gpio_cnt=%d < usbc_cnt=%d, clear count but not feedback\n", __func__, g_set_gpio_cnt, usbc_cnt);
		g_set_gpio_cnt = 0;
		clear_usbc_event_cnt();
	}

	if (g_get_audio_cnt != usbc_audio_cnt) {
		pr_info("%s: g_get_audio_cnt=%d, usbc_audio_cnt=%d, not equal\n", __func__, g_get_audio_cnt, usbc_audio_cnt);
		scnprintf(buf, sizeof(buf) - 1, "func@@%s$$payload@@usbc_switch notify audio count=%d,wcd get event count=%d,not equal", \
				__func__, usbc_audio_cnt, g_get_audio_cnt);
		upload_mm_fb_kevent_to_atlas_limit(OPLUS_AUDIO_EVENTID_HEADSET_DET, buf, MM_FB_KEY_RATELIMIT_1MIN);
		g_get_audio_cnt = 0;
		clear_usbc_audio_event_cnt();
	}
}
#endif /* CONFIG_OPLUS_FEATURE_MM_FEEDBACK */

static void wcd_usbc_dev_det_irq_handler(struct wcd_mbhc *mbhc)
{
	bool detection_type = 0;
	int ret = -1;
	...
	WCD_MBHC_REG_READ(WCD_MBHC_MECH_DETECTION_TYPE,
		detection_type);
	pr_info("%s: detection_type:%d\n", __func__, detection_type);

#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
	cancel_delayed_work_sync(&mbhc->check_count_dwrok);
	if (ktime_before(ktime_get(), ktime_add_ms(g_last_irq_tm, MIN_IRQ_INTERNAL_MS))) {
		g_set_gpio_cnt = 0;
		pr_info("%s: two plug action is too close, set g_set_gpio_cnt 0\n", __func__);
	} else {
		g_set_gpio_cnt++;
	}
	g_last_irq_tm = ktime_get();
#endif /* CONFIG_OPLUS_FEATURE_MM_FEEDBACK */

	if (detection_type == 1) {
		ret = gpio_direction_output(mbhc->usbc_insert_pin, 0);
		pr_info("%s: tregger cc-interrupt ret:%d\n", __func__, ret);
	} else {
		ret = gpio_direction_output(mbhc->usbc_insert_pin, 1);
		pr_info("%s: disable cc-interrupt ret:%d\n", __func__, ret);
	}
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
	schedule_delayed_work(&mbhc->check_count_dwrok, msecs_to_jiffies(CHECK_COUNT_DELAY_MS));
#endif /* CONFIG_OPLUS_FEATURE_MM_FEEDBACK */
...
#define pr_debug pr_info
	...
	if (unlikely((mbhc->mbhc_cb->lock_sleep(mbhc, true)) == false)) {
		...
		/* Call handler */
#if IS_ENABLED(CONFIG_OPLUS_HS_G_DET)
		if (mbhc->mbhc_cfg->usbc_hs_g_det_support == 1 &&
				gpio_is_valid(mbhc->usbc_insert_pin)) {
			wcd_usbc_dev_det_irq_handler(mbhc);
		} else {
...
#if IS_ENABLED(CONFIG_OPLUS_HS_G_DET)
		if (mbhc->mbhc_cfg->enable_usbc_analog &&
			!mbhc->mbhc_cfg->usbc_hs_g_det_support) {
...
static int wcd_mbhc_usbc_ana_detect_work_fn(struct notifier_block *nb,
					   unsigned long mode, void *ptr)
{
	struct wcd_mbhc *mbhc = container_of(nb, struct wcd_mbhc, aatc_dev_nb);
	int rc = 0;
...
int wcd_mbhc_start(struct wcd_mbhc *mbhc, struct wcd_mbhc_config *mbhc_cfg)
{
	...
	#endif /* OPLUS_ARCH_EXTENDS */
	#if IS_ENABLED(CONFIG_OPLUS_SEPARATE_USBC_SWITCH)
	mbhc_cfg->separate_aud_sw_support = 0;
	#endif
	#if IS_ENABLED(CONFIG_QCOM_FSA4480_I2C)
	mbhc_cfg->fsa4480_support = 0;
	#endif
	#if IS_ENABLED(CONFIG_OPLUS_HS_G_DET)
	mbhc_cfg->usbc_hs_g_det_support = 0;
	#endif
	...
	if (mbhc_cfg->enable_usbc_analog) {
		...
		if (!mbhc->aatc_dev_np && IS_ENABLED(CONFIG_QCOM_WCD_USBSS_I2C)) {
			mbhc->aatc_dev_np = of_parse_phandle(card->dev->of_node,
							"wcd939x-i2c-handle", 0);
			dev_dbg(mbhc->component->dev, "%s: wcd939x-i2c-handle is %s\n",
					__func__, mbhc->aatc_dev_np == NULL ? "NULL": "not NULL");
		}

		if (!mbhc->aatc_dev_np && IS_ENABLED(CONFIG_QCOM_FSA4480_I2C)) {
			mbhc->aatc_dev_np = of_parse_phandle(card->dev->of_node,
							"fsa4480-i2c-handle", 0);
			if (mbhc->aatc_dev_np) {
				mbhc_cfg->fsa4480_support = 1;
			}
			dev_dbg(mbhc->component->dev, "%s: fsa4480-i2c-handle is %s\n",
					__func__, mbhc->aatc_dev_np == NULL ? "NULL": "not NULL");
		}

		if (!mbhc->aatc_dev_np && IS_ENABLED(CONFIG_OPLUS_SEPARATE_USBC_SWITCH)) {
			mbhc->aatc_dev_np = of_parse_phandle(card->dev->of_node,
							"separate-usbc-switch", 0);
			if (mbhc->aatc_dev_np) {
				mbhc_cfg->separate_aud_sw_support = 1;
			}
			dev_dbg(mbhc->component->dev, "%s: separate_usbc_switch is %s\n",
					__func__, mbhc->aatc_dev_np == NULL ? "NULL": "not NULL");
		}

		if (!mbhc->aatc_dev_np) {
			dev_err(card->dev, "%s: mbhc->aatc_dev_np handle is not found int dt\n",
									__func__);
			rc = -EINVAL;
			goto err;
		}
...
#if IS_ENABLED(CONFIG_OPLUS_HS_G_DET)
	mbhc->usbc_insert_pin = of_get_named_gpio(card->dev->of_node,
			"hs-g-gpio", 0); //GOIO 133
	if (!gpio_is_valid(mbhc->usbc_insert_pin)) {
		pr_err("%s: usbc_insert_pin in dt node is missing\n", __func__);
	} else {
		rc = gpio_request(mbhc->usbc_insert_pin, "hs-g-gpio");
		if (rc) {
			pr_err("%s: usbc_insert_pin request fail\n", __func__);
		} else {
			pr_info("%s: usbc_insert_pin request init as high\n", __func__);
			gpio_direction_output(mbhc->usbc_insert_pin, 1);//default high
		}
	}

	if (of_find_property(card->dev->of_node, "hs-g-usbc-det-support", NULL)) {
		rc = of_property_read_u32(card->dev->of_node, "hs-g-usbc-det-support",
				&mbhc_cfg->usbc_hs_g_det_support);
		if (rc) {
			pr_err("%s: get HS_G state fail\n", __func__);
		} else {
			pr_info("%s: HS_G is support, usbc_hs_g_det_support=%d\n",
				__func__, mbhc_cfg->usbc_hs_g_det_support);
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
			INIT_DELAYED_WORK(&mbhc->check_count_dwrok, hs_g_check_count_fn);
#endif /* CONFIG_OPLUS_FEATURE_MM_FEEDBACK */
		}
	}
#endif
	...
	if (mbhc_cfg->enable_usbc_analog) {
		...
		if (mbhc->mbhc_cfg->separate_aud_sw_support &&
			mbhc->mbhc_cfg->usbc_hs_g_det_support) {
			mbhc->aatc_dev_nb.notifier_call = wcd_mbhc_usbc_ana_detect_work_fn;
		} else {
...
/* 2022/11/02, Add for separate audio switch */
		if (mbhc->mbhc_cfg->separate_aud_sw_support) {
			rc = usbc_switch_reg_notifier(&mbhc->aatc_dev_nb, mbhc->aatc_dev_np);
			if (rc) {
				pr_info("%s usbc_switch_reg_notifier fail,rc = %d", __func__, rc);
				rc = 0;
			}
			mbhc->need_cross_conn = 0;
			pr_info("%s rc = %d need_cross_conn set as 0", __func__, rc);
		}
#endif /* CONFIG_OPLUS_SEPARATE_USBC_SWITCH */
#if IS_ENABLED(CONFIG_QCOM_FSA4480_I2C)
		if (mbhc_cfg->fsa4480_support) {
			...
			if (!mbhc->need_cross_conn) {
				...
				mbhc->need_cross_conn = fsa4480_check_cross_conn(mbhc->aatc_dev_np);
...
/* 2022/11/02, Add for separate audio switch */
	if (mbhc->mbhc_cfg->enable_usbc_analog &&
		mbhc->mbhc_cfg->separate_aud_sw_support) {
		usbc_switch_unreg_notifier(&mbhc->aatc_dev_nb, mbhc->aatc_dev_np);
	}
#endif

#if IS_ENABLED(CONFIG_QCOM_FSA4480_I2C)
	if (mbhc->mbhc_cfg->enable_usbc_analog &&
		mbhc->mbhc_cfg->fsa4480_support) {
	...
	/*deinit aatc_dev_np, to fix issue after adsp reboot*/
	mbhc->aatc_dev_np = NULL;
