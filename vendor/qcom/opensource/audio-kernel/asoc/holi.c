	...
	if (!strcmp(match->data, "codec")) {
		...
		if (rc) {
			...
			if (mi2s_audio_intf) {
				...
				extend_codec_i2s_be_dailinks(dev, msm_mi2s_be_dai_links, ARRAY_SIZE(msm_mi2s_be_dai_links));
		...
		if (!rc && val) {
			#if IS_ENABLED(CONFIG_AUDIO_EXTEND_DRV)
			/*Add for oplus extend audio*/
			extend_codec_i2s_be_dailinks(dev, msm_tdm_be_dai_links, ARRAY_SIZE(msm_tdm_be_dai_links));
