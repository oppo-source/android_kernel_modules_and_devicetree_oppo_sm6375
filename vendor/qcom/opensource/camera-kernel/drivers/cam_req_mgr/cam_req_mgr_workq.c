...
#define SET_RTTHREAD_PROTITY(func, flags, para) \
  sched_##func(current, flags, para)
#endif
...
static int cam_req_mgr_thread(void *data)
{
	...
	SET_RTTHREAD_PROTITY(setscheduler_nocheck, SCHED_FIFO, &param);
	...
	int flags, void (*func)(struct work_struct *w))
{
	...
	if (!*workq) {
		...
		if (strstr(crm_workq->workq_name, "CRMCORE")) {
			...
			CAM_INFO(CAM_CRM, "enable %s workqueue thread %p", crm_workq->workq_name, crm_workq->thread);
			if (IS_ERR(crm_workq->thread) || (NULL == crm_workq->thread)) {
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		if ((*crm_workq)->thread)  {
			CAM_INFO(CAM_CRM, "stop %s workqueue thread %p",
				(*crm_workq)->workq_name, (*crm_workq)->thread);
			mutex_lock(&(*crm_workq)->rt_lock);
