...
static void cam_mem_mgr_print_tbl(void)
{
	...
	CAM_INFO(CAM_MEM, "***%llu:%llu:%llu:%llu Mem mgr table dump***", hrs,
		 min, sec, ms);
	for (i = 1; i < CAM_MEM_BUFQ_MAX; i++) {
		CAM_CONVERT_TIMESTAMP_FORMAT((tbl.bufq[i].timestamp), hrs, min,
					     sec, ms);
		CAM_INFO(
			CAM_MEM,
			"%llu:%llu:%llu:%llu idx %d fd %d size %llu active %d buf_handle "
			"%d refCount %d",
			hrs, min, sec, ms, i, tbl.bufq[i].fd, tbl.bufq[i].len,
			tbl.bufq[i].active, tbl.bufq[i].buf_handle,
...
void cam_mem_put_cpu_buf(int32_t buf_handle)
{
	...
	if (buf_handle != tbl.bufq[idx].buf_handle) {
		CAM_ERR(CAM_MEM, "idx: %d Invalid buf handle %d", idx,
			buf_handle);
		rc = -EINVAL;
		return;
	}
	...
	if (kref_put(&tbl.bufq[idx].krefcount, cam_mem_util_unmap)) {
		...
		CAM_CONVERT_TIMESTAMP_FORMAT((tbl.bufq[idx].timestamp), hrs,
					     min, sec, ms);
		CAM_ERR(CAM_MEM,
			"%llu:%llu:%llu:%llu idx %d fd %d size %llu active %d buf_handle "
			"%d refCount %d",
			hrs, min, sec, ms, idx, tbl.bufq[idx].fd,
			tbl.bufq[idx].len, tbl.bufq[idx].active,
			tbl.bufq[idx].buf_handle,
			kref_read(&tbl.bufq[idx].krefcount));
		CAM_GET_TIMESTAMP(current_ts);
		CAM_CONVERT_TIMESTAMP_FORMAT(current_ts, hrs, min, sec, ms);
		CAM_ERR(CAM_MEM,
			"%llu:%llu:%llu:%llu  Not unmapping even after defer, buf_handle: "
			"%u, idx: %d",
			hrs, min, sec, ms, buf_handle, idx);
	}
}
