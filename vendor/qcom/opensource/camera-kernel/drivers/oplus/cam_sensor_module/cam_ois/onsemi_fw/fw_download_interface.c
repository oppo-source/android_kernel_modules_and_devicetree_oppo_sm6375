...
int StoreOisGyroGian(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	else if(strstr(o_ctrl->ois_name, "bu24721"))
	{
  	...
  	else if (strstr(o_ctrl->ois_name, "sem1217s"))
  	{
		...
		DW9785_StoreGyroGainToFlash(o_ctrl);
	}
	return rc;
}
...
int DoDW9785GyroOffset(struct cam_ois_ctrl_t *o_ctrl, uint32_t *gyro_data)
{
	int rc = 0;
	uint32_t ReadGyroOffset_X = 0, ReadGyroOffset_Y = 0;
	struct _FACT_ADJ_DW9785 FADJCAL = { 0x0000, 0x0000};
	...
	FADJCAL= dw9785_gyro_ofs_calibration(o_ctrl);
...
int DoOisAgingSetThread(void *pctrl)
{
	int rc = 0;
	struct cam_ois_ctrl_t *o_ctrl = (struct cam_ois_ctrl_t *)pctrl;
	if (!o_ctrl)
	{
		CAM_ERR(CAM_OIS, "Invalid Args");
		return -EINVAL;
	...
	if (o_ctrl->oisaging_set_thread == 1) {
		if (o_ctrl->oisaging_work_thread) {
			CAM_WARN(CAM_OIS, "OISAging: thread already exist");
		} else {
			CAM_INFO(CAM_OIS, "OISAging: thread init");
			init_waitqueue_head(&(o_ctrl->oisaging_work_wq));

			o_ctrl->oisaging_work_thread =
				kthread_run(DoOisAgingSetTarget, (void *)o_ctrl, "oisaging_thread");
			CAM_ERR(CAM_OIS, "OISAging: thread run");

			if (IS_ERR(o_ctrl->oisaging_work_thread)) {
				//handle error
				rc = PTR_ERR(o_ctrl->oisaging_work_thread);
				CAM_ERR(CAM_OIS, "OISAging: thread error %d", rc);
				return rc;
			}
		}
	} else if (o_ctrl->oisaging_set_thread == 0) {
		if (o_ctrl->oisaging_work_thread) {
			CAM_INFO(CAM_OIS, "OISAging: thread stop");
			while (true) {
				if (mutex_trylock(&(o_ctrl->oisaging_work_mutex))) {
					o_ctrl->oisaging_need_work = 0;
					mutex_unlock(&(o_ctrl->oisaging_work_mutex));
					break;
				}
				usleep_range(1000, 1005); // wait for 1ms to get lock
			}

			o_ctrl->oisaging_work_thread = NULL;
		} else {
			CAM_WARN(CAM_OIS, "OISAging: thread already stopped");
		}
	}
...
int DoOisAgingSetTarget(void *pctrl)
{
	int rc = 0;
	struct cam_ois_ctrl_t *o_ctrl = (struct cam_ois_ctrl_t *)pctrl;
	...
	while (true) {
		wait_event_interruptible(o_ctrl->oisaging_work_wq,
			o_ctrl->oisaging_need_work == 1 || o_ctrl->oisaging_set_thread == 0);
		CAM_INFO(CAM_OIS, "OISAging: thread wake trigger need work=%d, set thread=%d",
			o_ctrl->oisaging_need_work, o_ctrl->oisaging_set_thread);

		while (true) {
			if (mutex_trylock(&(o_ctrl->oisaging_work_mutex))) {
				o_ctrl->oisaging_need_work = 0;
				if (strstr(o_ctrl->ois_name, "dw9785")) {
					dw9785_aging_set_target(o_ctrl);
				}
				mutex_unlock(&(o_ctrl->oisaging_work_mutex));
				break;
			}
		...
		if (o_ctrl->oisaging_set_thread == 0) {
			CAM_INFO(CAM_OIS, "OISAging: thread need stop");
			break;
		}
	}
...
int DoOisAgingGetActual(struct cam_ois_ctrl_t *o_ctrl, uint32_t get_actual[], uint32_t size)
{
	int rc = 0;
	int i = 0;
	uint32_t ois_actual[OIS_AGING_LOOP_DATA_NUM] = { 0 };
	size = OIS_AGING_LOOP_DATA_NUM;
	...
	if (strstr(o_ctrl->ois_name, "dw9785")) {
		dw9785_aging_get_actual(ois_actual, size);
	}

	for (i = 0; i < OIS_AGING_LOOP_DATA_NUM; i++) {
		get_actual[i] = ois_actual[i];
	}
	return rc;
}

int DoOisAgingLoopDone(struct cam_ois_ctrl_t *o_ctrl, uint32_t *loop_done)
{
	int rc = 0;
	if (!o_ctrl) {
		CAM_ERR(CAM_OIS, "Invalid Args");
	...
	if (strstr(o_ctrl->ois_name, "dw9785")) {
		dw9785_aging_loopdone(loop_done);
	}
...
int WriteOisGyroGian(struct cam_ois_ctrl_t *o_ctrl,OIS_GYROGAIN* current_gyro_gain)
{
	...
	if(strstr(o_ctrl->ois_name,"bu24721")){
		...
		DW9785_WriteGyroGainToFlash(o_ctrl,current_gyro_gain->GyroGain_X , current_gyro_gain->GyroGain_Y);
	}
...
int OIS_READ_HALL_DATA_TO_UMD_NEW (struct cam_ois_ctrl_t *o_ctrl,struct i2c_settings_array *i2c_settings) {
	...
	list_for_each_entry(i2c_list,
		...
		if ((read_buff == NULL) || (buff_length == 0)) {
			CAM_ERR(CAM_OIS, "Invalid input buffer, buffer: %pK, length: %d", read_buff, buff_length);
			return -EINVAL;
		}
		...
		&(i2c_settings->list_head), list)
	{
		...
		if(fifo_len_in_ois_driver>0)
		{
			...
			if(sample_count>0){
				...
				for (j = 0; j < 8; j++) {
					CAM_DBG(CAM_OIS, "time: reg_data[%d]: 0x%x", 135-j, (qtimer & 0xFF));
					read_buff[135-j] = (qtimer & 0xFF);
					qtimer >>= 8;
				}
				...
				read_buff[143] = 0;
				for (j = 0; j < 8; j++) {
					CAM_DBG(CAM_OIS, "time: reg_data[%d]: 0x%x", 135-j, (pre_qtimer & 0xFF));
					read_buff[135-j] = (pre_qtimer & 0xFF);
					pre_qtimer >>= 8;
				}
		}
		mutex_unlock(&(o_ctrl->ois_hall_data_mutex));
...
int OIS_READ_HALL_DATA_TO_UMD_129 (struct cam_ois_ctrl_t *o_ctrl,struct i2c_settings_array *i2c_settings) {
	...
	list_for_each_entry(i2c_list,
		...
		buff_length = i2c_list->i2c_settings.read_buff_len;
		if ((read_buff == NULL) || (buff_length == 0)) {
			CAM_DBG(CAM_OIS, "Invalid input buffer, buffer: %pK, length: %d", read_buff, buff_length);
			return -EINVAL;
		}
		read_length = i2c_list->i2c_settings.size;
		temp_buff = kzalloc(read_length, GFP_KERNEL);
...
int OIS_READ_HALL_DATA_TO_UMD_SEM1217S (struct cam_ois_ctrl_t *o_ctrl,struct i2c_settings_array *i2c_settings)
{
	...
	list_for_each_entry(i2c_list,
		...
		CAM_DBG(CAM_OIS,"read done fifo_count %x, start_qtimer: %lld ns, end_qtimer: %lld ns laps %lld ns",
			fifo_count, qtimer_ns, qtimer_ns_1, (qtimer_ns-qtimer_ns_1));
...
bool IsOISReady(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl)
	{
		do
		{
			...
			{
				rc = camera_io_dev_read(&(o_ctrl->io_master_info), 0xD016, (uint32_t *)&ReadVal,
						CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_WORD);
				if (rc < 0) {
					CAM_ERR(CAM_OIS, "[DW9785] Read Addr_OIS_STS(0xD016) failed, retry");
				}
				else
				{
					CAM_INFO(CAM_OIS, "[DW9785] Read Addr_OIS_STS(0xD016) = 0x%x", ReadVal);
					if (ReadVal == 0x9785)/* STATE_READY or STATE_RUN*/
					{
						CAM_INFO(CAM_OIS, "OIS %d IsOISReady", o_ctrl->ois_type);
						ois_state[o_ctrl->ois_type] = CAM_OIS_READY;
						return true;
...
void DeinitOIS(struct cam_ois_ctrl_t *o_ctrl)
{
	if (o_ctrl)
	{
		...
		if ((o_ctrl->oisaging_work_thread != NULL) ||
			(o_ctrl->oisaging_set_thread == 1))
		{
			o_ctrl->oisaging_set_thread = 0;
...
int update_ois_firmware_dw9785(void *arg)
{
	struct cam_ois_ctrl_t *o_ctrl = (struct cam_ois_ctrl_t *)arg;
	int rc = 0;
	CAM_INFO(CAM_OIS, "update_ois_firmware_dw9785");
	o_ctrl->io_master_info.master_type = CCI_MASTER;
	...
	rc = dw9785_download_fw(o_ctrl);

	cam_ois_power_down(o_ctrl);
	CAM_INFO(CAM_OIS, "exit update_ois_firmware_dw9785 rc:%d", rc);
	return rc;
}
...
void InitOISResource(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl)
	{
		...
		if (strstr(o_ctrl->ois_name, "dw9785"))
		{
			CAM_ERR(CAM_OIS, "create dw9785 ois download fw thread for dw9785");
			o_ctrl->ois_downloadfw_thread = kthread_run(update_ois_firmware_dw9785, o_ctrl, o_ctrl->ois_name);
		}
...
int32_t oplus_cam_ois_construct_default_power_setting_dw9785(
	struct cam_sensor_power_ctrl_t *power_info)
{
	int rc = 0;
...
int sem1217_block_write(void* register_data,int size)
{
	...
	for(i = 0; i < retry; i++)
	{
		rc = camera_io_dev_write_continuous(&(ois_ctrl->io_master_info), &i2c_write, 1);
		if (rc < 0)
		{
	...
	if (strstr(o_ctrl->ois_name, "bu24721_tele"))
	{
		if (o_ctrl->pre_isTeleOisUseMonitor != o_ctrl->isTeleOisUseMonitor)
		{
			...
			if (o_ctrl->pre_isTeleOisUseMonitor)
			{
				...
				I2C_OIS_8bit_write(o_ctrl,0xF02A, 0x04);//monitor mode
			}
			else
			{
				CAM_INFO(CAM_OIS, "spi_mode_switch from master to monitor");
				I2C_OIS_8bit_write(o_ctrl,0xF02A, 0x00);//master mode
			}
			F024_Polling(o_ctrl);
			I2C_OIS_8bit_write(o_ctrl,0xF023, 02);//gyro read off
			F024_Polling(o_ctrl);
			Gyro_select(o_ctrl,Q_ICM42631, o_ctrl->isTeleOisUseMonitor);
		}
	}
...
int dw9785_i2c_block_write_reg(struct cam_ois_ctrl_t *o_ctrl, unsigned short addr, unsigned short *dat, int size )
{
	...
	{
		data =  dat[i];
		rc=write_reg_16bit_value_16bit(o_ctrl, addr, data);//addr = 0xDE05
		if(rc < 0) {
			CAM_ERR(CAM_OIS, "dw9785_i2c_block_write_reg() fail");
		}
	}
	return rc;
}

int read_ois_data_dw9785(void *o_ctrl)
{
	int rc=0;
	uint32_t gyro_x,gyro_y,gyro_offset_x,gyro_offset_y,gyro_gain_x,gyro_gain_y,hall_x,hall_y,target_x,target_y,position_x, position_y;
	uint16_t Addr1,Addr2,Addr3,Addr4,Addr5,Addr6,Addr7,Addr8,Addr9,AddrA,AddrB,AddrC;
	Addr1= 0X7120;//gyro_x
	Addr2= 0X71A0;//gyro_y
	Addr3= 0X7700;//gyro_offset_x
	Addr4= 0X7701;//gyro_offset_y
	Addr5= 0X7702;//gyro_gain_x
	Addr6= 0X7703;//gyro_gain_y
	Addr7= 0x7102;//hall_x
	Addr8= 0x7182;//hall_y
	Addr9= 0x7106;//target_x
	AddrA= 0x7186;//target_y
	AddrB= 0x7101;//position_x
	AddrC= 0x7181;//position_y

	CAM_ERR(CAM_OIS, "is_dw9785 %d",is_dw9785);
	while(is_dw9785)
	{
	rc = read_reg_16bit_value_16bit(o_ctrl, Addr1, &gyro_x);
	rc = read_reg_16bit_value_16bit(o_ctrl, Addr2, &gyro_y);
	rc = read_reg_16bit_value_16bit(o_ctrl, Addr3, &gyro_offset_x);
	rc = read_reg_16bit_value_16bit(o_ctrl, Addr4, &gyro_offset_y);
	rc = read_reg_16bit_value_16bit(o_ctrl, Addr5, &gyro_gain_x);
	rc = read_reg_16bit_value_16bit(o_ctrl, Addr6, &gyro_gain_y);
	rc = read_reg_16bit_value_16bit(o_ctrl, Addr7, &hall_x);
	rc = read_reg_16bit_value_16bit(o_ctrl, Addr8, &hall_y);
	rc = read_reg_16bit_value_16bit(o_ctrl, Addr9, &target_x);
	rc = read_reg_16bit_value_16bit(o_ctrl, AddrA, &target_y);
	rc = read_reg_16bit_value_16bit(o_ctrl, AddrB, &position_x);
	rc = read_reg_16bit_value_16bit(o_ctrl, AddrC, &position_y);
	CAM_ERR(CAM_OIS, "gyro_x %d gyro_y %d gyro_offset_x %d gyro_offset_y %d gyro_gain_x %d gyro_gain_y %d hall_x %d hall_y %d target_x %d target_y %d position_x %d position_y %d",
		gyro_x,gyro_y,gyro_offset_x,gyro_offset_y,gyro_gain_x,gyro_gain_y,hall_x,hall_y,target_x,target_y,position_x,position_y);
	mdelay(20);
	}
	return rc;
}
