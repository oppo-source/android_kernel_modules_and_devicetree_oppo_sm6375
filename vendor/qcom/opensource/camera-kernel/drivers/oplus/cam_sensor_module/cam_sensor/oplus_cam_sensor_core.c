	...
	case CAM_OEM_IO_CMD:{
		...
		if (oem_ctl.num_bytes > 0) {
			...
			if (copy_from_user(cam_regs, u64_to_user_ptr(oem_ctl.cam_regs_ptr),
				...
				CAM_INFO(CAM_SENSOR, "copy_from_user error!!!");
				rc = -EFAULT;
				goto free_cam_regs;
			}
		}
		...
		case CAM_OEM_OIS_CALIB : {
			/*rc = cam_ois_sem1215s_calibration(&oem_io_master_info);*/
                      CAM_ERR(CAM_SENSOR, "no need ois calib ");
                      break;
		}

		default:
			CAM_ERR(CAM_SENSOR,"Unknow OEM cmd ,slave sensor id is 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			break ;
		}
...
int cam_sensor_match_id_by_vcmid(struct cam_sensor_ctrl_t *s_ctrl)
{
	int rc = 0;
	uint32_t vcmid_addr = 0x0A;
	uint32_t vcmid_data = 0;
	struct cam_camera_slave_info *slave_info;
	...
	if (slave_info->vcm_id != 0)
	{
		...
		if (vcmid_data == slave_info->vcm_id)
		{
			return rc;
		}
	}
