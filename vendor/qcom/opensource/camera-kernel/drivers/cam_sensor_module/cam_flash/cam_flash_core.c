	...
	bool regulator_enable)
{
	...
	if (regulator_enable && (fctrl->is_regulator_enabled == false)) {
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		// add for pmic sc6607
		if (fctrl->pmic_sc6607) {
			CAM_ERR(CAM_FLASH, "[SC6607] NOP power up settings.");
		...
		// add for pmic sc6607
		if (fctrl->pmic_sc6607) {
			CAM_ERR(CAM_FLASH, "[SC6607] NOP power down settings.");
	...
	if (req_id == 0) {
		...
		if (fctrl->apply_streamoff == true) {
			...
			list_for_each_entry(i2c_list,
				...
				// add for pmic sc6607
				if (fctrl->pmic_sc6607) {
					rc = cam_flash_sc6607_i2c_apply_setting
						(fctrl, &(i2c_list->i2c_settings));
				} else {
		...
		if ((i2c_set->is_settings_valid == true) &&
			(i2c_set->request_id == req_id)) {
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		if(!strcmp(fctrl->flash_name, "i2c_flash") && (false == fctrl->pmic_sc6607)) {
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
	// add for sc6607
	if((csl_packet->header.op_code & 0xFFFFFF) == CAM_FLASH_PACKET_OPCODE_NEED_BOOST_EN){
		CAM_ERR(CAM_FLASH, "[SC6607] CAM_FLASH_PACKET_OPCODE_NEED_BOOST_EN E");
		fctrl->sc6607_boost = true;
		CAM_ERR(CAM_FLASH, "[SC6607] oplus_chg_set_camera_on CALL OPEN E");
		//oplus_chg_set_camera_on(1);
		CAM_ERR(CAM_FLASH, "[SC6607] oplus_chg_set_camera_on CALL OPEN X");
	}
#endif
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
	case CAM_FLASH_PACKET_OPCODE_NEED_BOOST_EN:
#endif
...
update_req_mgr:
	...
	if (((csl_packet->header.op_code  & 0xFFFFF) ==
		...
		((csl_packet->header.op_code & 0xFFFFF) ==
		CAM_FLASH_PACKET_OPCODE_NEED_BOOST_EN)) {
#else
	if (((csl_packet->header.op_code  & 0xFFFFF) ==
		CAM_PKT_NOP_OPCODE) ||
		((csl_packet->header.op_code & 0xFFFFF) ==
