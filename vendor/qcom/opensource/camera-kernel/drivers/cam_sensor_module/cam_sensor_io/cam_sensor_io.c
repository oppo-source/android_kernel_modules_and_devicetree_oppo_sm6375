	...
	struct cam_sensor_i2c_reg_setting *write_setting)
{
	...
	CAM_DBG(CAM_SENSOR, "master_type: %d, register_addr: %x, value_to_write:%d",
			io_master_info->master_type,
			write_setting->reg_setting->reg_addr,
			write_setting->reg_setting->reg_data);
	if (write_setting->reg_setting->reg_addr == 0xD004 && write_setting->reg_setting->reg_data == 0x9785) {
		int32_t rc = 0;
		rc = camera_io_dev_read(io_master_info,
								...
								CAMERA_SENSOR_I2C_TYPE_WORD);
		if(rc < 0) {
			CAM_ERR(CAM_SENSOR, "camera_io_dev_read Reg_addr: 0xD004 fail!");
		} else {
			CAM_DBG(CAM_SENSOR, "camera_io_dev_read Reg_addr:, value= %d", write_setting->reg_setting->reg_data);
		}
		return rc;
	}
#endif
