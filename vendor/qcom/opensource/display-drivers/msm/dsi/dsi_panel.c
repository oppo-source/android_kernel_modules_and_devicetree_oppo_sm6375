...
static int dsi_panel_reset(struct dsi_panel *panel)
{
...
#ifdef OPLUS_FEATURE_DISPLAY
		if (!strcmp(panel->oplus_priv.vendor_name , "A0017")) {
			usleep_range(2*1000, (2*1000)+100);
		}
		if (!strcmp(panel->name, "AC172 P 7 A0001 dsc cmd mode panel")
			|| !strcmp(panel->name, "AC172 P 3 A0004 dsc cmd mode panel")) {
	...
	for (i = 0; i < r_config->count; i++) {
		...
		if (!strcmp(panel->name, "AC172 P 3 A0023 dsc cmd mode panel") &&
			already_readid && panel_id.DA == 0x55 && panel_id.DB == 0x81) {
			if (i == 0 || i == 1) {
				continue;
			}
		}
...
#ifdef OPLUS_FEATURE_DISPLAY
	if (!strcmp(panel->name, "AC172 P 3 A0004 dsc cmd mode panel")) {
		usleep_range(10000, 10100);
	}
#endif /* OPLUS_FEATURE_DISPLAY */
...
/*#ifdef OPLUS_FEATURE_TP_BASIC*/
	if (!strcmp(panel->name, "td4377 video mode dsi fhd 1080 panel")) {
		OPLUS_PANEL_EVENT_NOTIFY(0, DRM_PANEL_EVENT_BLANK, 0x00)
	}
/*#endif OPLUS_FEATURE_TP_BASIC*/
...
/* #ifdefine OPLUS_FEATURE_DISPLAY */
	usleep_range(20000, 20100);
        ...
        if (!strcmp(panel->name,"nt36672c video mode dsi fhd 1080 panel")
        || !strcmp(panel->name,"td4377 video mode dsi fhd 1080 panel")) {
...
#ifdef OPLUS_FEATURE_DISPLAY
	/* AC172 P 3 A0004: add shutdown sequence */
	if (!strcmp(panel->name, "AC172 P 3 A0004 dsc cmd mode panel")) {
		DSI_INFO("[%s] AC172 P 3 A0004 shutdown_flag = %d\n", __func__, shutdown_flag);
		if (1 == shutdown_flag) {
...
/*#ifdef OPLUS_FEATURE_TP_BASIC*/
	if (!strcmp(panel->name,"nt36672c video mode dsi fhd 1080 panel")
		...
		if((is_pd_with_guesture == false) || (1 == shutdown_flag) || panel_esd_check_failed) {
			...
			DSI_INFO("[%s][TP] tp gesture is disable, or will shutdown or esd_check failed! need to disable tp-irq and goto power off, tp_irq:%d\n", __func__, tp_irq);
...
#endif /* OPLUS_FEATURE_DISPLAY */
{
	...
	mode = panel->cur_mode;
#ifdef OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT
	if (oplus_ofp_is_supported() && oplus_ofp_video_mode_aod_fod_is_enabled() && mode->timing.refresh_rate == 60) {
		switch (type) {
			case DSI_CMD_HBM_ON:
...
SDE_ATRACE_BEGIN(cmd_set_prop_map[type]);
	...
	for (i = 0; i < count; i++) {
		...
		if (type == DSI_CMD_SET_VID_SWITCH_OUT ||
			...
			type == DSI_CMD_ESD_SWITCH_DEFAULT_PAGE )
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-esd-switch-default-page-command-state",
				...
				struct dsi_parser_utils *utils)
{
	...
	rc = sde_dsc_populate_dsc_private_params(&priv_info->dsc, intf_width, priv_info->widebus_support);
	if (rc) {
...
#ifdef OPLUS_FEATURE_DISPLAY
	esd_config->esd_default_cmds_switch_enabled = utils->read_bool(utils->data,
