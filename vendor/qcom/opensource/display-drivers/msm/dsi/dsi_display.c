	...
	display_for_each_ctrl(i, display) {
		...
		if (display->panel->panel_mode == DSI_OP_VIDEO_MODE) {
			flags |= DSI_CTRL_CMD_CUSTOM_DMA_SCHED;
                ...
                if ((atomic_read(&display->panel->vidmode_backlight_async_wait)) && (((unsigned char*)(msg->tx_buf))[0] == 0x51)) {
...
static int dsi_display_read_status(struct dsi_display_ctrl *ctrl,
		struct dsi_display *display)
{
	int i, rc = 0, count = 0, start = 0, *lenp, ret = 0;
	struct drm_panel_esd_config *config;
...
#ifdef OPLUS_FEATURE_DISPLAY
		if(!strcmp(panel->name,"AC172 P 3 A0023 dsc cmd mode panel") && i == 0){
...
#ifdef OPLUS_FEATURE_DISPLAY
	if (config->esd_default_cmds_switch_enabled) {
		ret = dsi_panel_tx_cmd_set(panel, DSI_CMD_ESD_SWITCH_DEFAULT_PAGE);
		if (ret) {
			DSI_ERR("[%s] failed to send DSI_CMD_ESD_SWITCH_DEFAULT_PAGE, ret=%d\n", panel->name, ret);
			return ret;
		}
	}
#endif /* OPLUS_FEATURE_DISPLAY */
