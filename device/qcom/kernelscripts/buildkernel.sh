...
PGOFLAGS=
KERNEL_ABS_PATH=$(readlink -m ${KERNEL_DIR})
...
make_defconfig()
{
	if [ -z "${SKIP_DEFCONFIG}" ] ; then
        ...
        if [ ! -z "${KERNEL_CONFIG_OVERRIDE}" ]; then
            ...
            echo ${KERNEL_CONFIG_OVERRIDE} >> ${OUT_DIR}/.config;
            ${MAKE_PATH}make -C ${KERNEL_DIR} O=${OUT_DIR} ${MAKE_ARGS} HOSTCFLAGS="${TARGET_INCLUDES}" HOSTLDFLAGS="${TARGET_LINCLUDES}" ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} oldconfig;
        fi;
#endif OPLUS_FEATURE_FORCE_SELINUX
		set +x
	fi
	...
	if [ "${KERNEL_PGO_FLAG}" -eq 1 ]; then
		...
		PGOFLAGS="-fprofile-use=${KERNEL_ABS_PATH}/pgo-profiles/vmlinux_v1.profdata -Wno-backend-plugin -Wno-profile-instr-unprofiled -Wno-profile-instr-out-of-date"
		${KERNEL_DIR}/scripts/config --file ${OUT_DIR}/.config \
			-e ARCH_SUPPORTS_PGO_CLANG \
			-d PGO_CLANG
	else
	...
	echo " pgo cflags: ${PGOFLAGS}"
	(cd ${KERNEL_DIR} && \
		${MAKE_PATH}make O=${OUT_DIR} ${MAKE_ARGS} KCFLAGS_PGO="${PGOFLAGS}" HOSTCFLAGS="${TARGET_INCLUDES}" HOSTLDFLAGS="${TARGET_LINCLUDES}" ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} olddefconfig)
	set +x
}
...
build_kernel()
{
	...
	(cd ${OUT_DIR} && \
	${MAKE_PATH}make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} KCFLAGS_PGO="${PGOFLAGS}" HOSTCFLAGS="${TARGET_INCLUDES}" HOSTLDFLAGS="${TARGET_LINCLUDES}" O=${OUT_DIR} ${CC_ARG} ${MAKE_ARGS} -j${NCORES})
	set +x
}
...
copy_all_to_prebuilt()
{
	...
	if [ -f ${OUT_DIR}/${IMAGE_FILE_PATH}/dts/vendor/qcom/*.dtb ]; then
		cp -p -r ${OUT_DIR}/${IMAGE_FILE_PATH}/dts/vendor/qcom/*.dtb ${PREBUILT_OUT}/${IMAGE_FILE_PATH}/dts/vendor/qcom/
	fi
