...
ifneq ($(GKI_KERNEL_DEFCONFIG),true)

ifneq ($(TARGET_BOARD_AUTO),true)
     ifeq ($(TARGET_BUILD_VARIANT),user)
     KERNEL_DEFCONFIG := vendor/$(TARGET_BOARD_PLATFORM)-qgki_defconfig
     else
     ...
     KERNEL_DEFCONFIG := vendor/$(TARGET_BOARD_PLATFORM)-gki_defconfig
endif
$(info ###### GKI_KERNEL_DEFCONFIG is $(GKI_KERNEL_DEFCONFIG) ######)
$(info ###### KERNEL_DEFCONFIG is $(KERNEL_DEFCONFIG) ######)
...
KERNEL_SRC_DEP := $(foreach c_dir, $(shell find $(SOURCE_ROOT)/$(TARGET_KERNEL_SOURCE)/ -name "\.git"|sed 's/\.git//g'), $(subst $(SPACE) , $(SPACE)$(c_dir),  $(shell cd ${c_dir};git ls-files|sed "s/\(.*\)\/.*/\1/g"|sort -u)))
KERNEL_SRC_DEP := $(filter-out $(SOURCE_ROOT)/$(TARGET_KERNEL_SOURCE)/arch/arm64/configs $(SOURCE_ROOT)/$(TARGET_KERNEL_SOURCE)/arch/arm64/configs/vendor, $(KERNEL_SRC_DEP))
KERNEL_SRC_DEP += $(SOURCE_ROOT)/$(TARGET_KERNEL_SOURCE)/arch/arm64/configs/$(KERNEL_DEFCONFIG)
...
#ifdef OPLUS_FEATURE_BUILD
kernel_defconfig_update_flag := $(PRODUCT_OUT)/.kernel_defconfig_$(TARGET_BUILD_VARIANT).oplus_update_flag
ifeq ($(OPLUS_FORCE_UPDATE_MAKEFILE),true)
$(shell rm -rf $(kernel_defconfig_update_flag))
endif
ifeq ($(wildcard $(kernel_defconfig_update_flag)),)
# Generate the defconfig file from the fragments
#ifdef VENDOR_EDIT
_x := $(shell ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(real_cc) KERN_OUT=$(KERNEL_OUT) HOSTCC=$(hostcc) $(TARGET_KERNEL_MAKE_ENV) MAKE_PATH=$(MAKE_PATH) TARGET_BUILD_VARIANT=${TARGET_BUILD_VARIANT} $(TARGET_KERNEL_SOURCE)/scripts/gki/generate_defconfig.sh $(KERNEL_DEFCONFIG))
$(shell touch $(kernel_defconfig_update_flag))
#endif
endif
#endif
else
...
ifeq ($(GKI_KERNEL),1)
  #If the configuration is QGKI(user or userdebug), build the GKI kernel as well
  #ifeq "$(KERNEL_DEFCONFIG)" "vendor/$(TARGET_BOARD_PLATFORM)-qgki_defconfig"
  ifneq (,$(findstring -qgki,$(KERNEL_DEFCONFIG)))
    ...
    #ifdef OPLUS_FEATURE_BUILD
    gki_kernel_defconfig_update_flag := $(PRODUCT_OUT)/.gki_kernel_defconfig_$(TARGET_BUILD_VARIANT).oplus_update_flag
    ifeq ($(OPLUS_FORCE_UPDATE_MAKEFILE),true)
    $(shell rm -rf $(gki_kernel_defconfig_update_flag))
    endif
    ifeq ($(wildcard $(gki_kernel_defconfig_update_flag)),)
        # Generate the GKI defconfig
    #ifdef VENDOR_EDIT 
        _x := $(shell ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(real_cc) KERN_OUT=$(KERNEL_OUT) HOSTCC=$(hostcc) $(TARGET_KERNEL_MAKE_ENV) MAKE_PATH=$(MAKE_PATH) TARGET_BUILD_VARIANT=${TARGET_BUILD_VARIANT} $(TARGET_KERNEL_SOURCE)/scripts/gki/generate_defconfig.sh $(GKI_KERNEL_DEFCONFIG))
    $(shell touch $(gki_kernel_defconfig_update_flag))
    #endif
    endif
    #endif
  endif
...
define build-kernel
	...
	VENDOR_RAMDISK_KERNEL_MODULES="$(VENDOR_RAMDISK_KERNEL_MODULES)" \
	TARGET_PRODUCT=$(TARGET_BOARD_PLATFORM) \
	KERNEL_PGO_FLAG=$(KERNEL_PGO_FLAG) \
...
$(KERNEL_HEADERS_INSTALL): $(KERNEL_OUT) $(DTC) $(UFDT_APPLY_OVERLAY)
	AGING_DEBUG_MASK=$(AGING_DEBUG_MASK) \
...
$(TARGET_PREBUILT_KERNEL): $(DTC) $(KERNEL_USR) $(KERNEL_SRC_DEP)
	mkdir -p $(KERNEL_OUT)
	echo "Building the requested kernel.."; \
	AGING_DEBUG_MASK=$(AGING_DEBUG_MASK) \
	$(call build-kernel,$(KERNEL_DEFCONFIG),$(KERNEL_OUT),$(KERNEL_MODULES_OUT),$(KERNEL_HEADERS_INSTALL),0,$(TARGET_PREBUILT_INT_KERNEL))
...
ifeq ( , true)
# Check GKI KMI as well when building GKI kernel
$(GKI_TARGET_PREBUILT_KERNEL): $(DTC) $(UFDT_APPLY_OVERLAY) $(GKI_KERNEL_OUT)
	...
	echo "Check GKI KMI.."; \
	$(SOURCE_ROOT)/device/qcom/kernelscripts/abi/Check_KMI.sh $(TARGET_BOARD_PLATFORM)
else
$(GKI_TARGET_PREBUILT_KERNEL): $(DTC) $(UFDT_APPLY_OVERLAY) $(KERNEL_SRC_DEP)
	mkdir -p $(GKI_KERNEL_OUT)
	echo "Building GKI kernel.."; \
	$(call build-kernel,$(GKI_KERNEL_DEFCONFIG),$(GKI_KERNEL_OUT),$(GKI_KERNEL_MODULES_OUT),$(GKI_KERNEL_HEADERS_INSTALL),0,$(GKI_TARGET_PREBUILT_INT_KERNEL))
	echo "Check GKI KMI.."; \
